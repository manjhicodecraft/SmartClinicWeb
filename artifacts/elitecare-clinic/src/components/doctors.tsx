...
function DoctorAvatar({ name }: { name: string }) {
  ...
  const colors = ["from-sky-500 to-blue-600", "from-emerald-500 to-teal-600", "from-purple-500 to-violet-600", "from-rose-500 to-pink-600", "from-amber-500 to-orange-600", "from-indigo-500 to-blue-600"];
...
export function Doctors({ onBookClick }: { onBookClick: () => void }) {
  ...
  return (
    <section id="doctors" className="py-24 bg-slate-950 dark:bg-slate-950">
              ...
              >
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <h3 className="font-display font-bold text-white truncate">{doctor.name}</h3>
                ...
                <div className="flex items-center justify-between">
                  <Badge
                    ...
                    variant="outline"
                  >
                    <span className={`mr-1.5 inline-block w-1.5 h-1.5 rounded-full ${doctor.availabilityStatus === "available" ? "bg-emerald-400 animate-pulse" : "bg-amber-400"}`} />
                    {doctor.availabilityStatus === "available"
                      ? "Available Now"
